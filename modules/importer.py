...
class DataImport:
    ...
    def create_dummy_data():
        ...
        for _ in range(n_rows):
            # Each job posting has 5-15 random skills, but not more than the total number of skills available
            max_possible_skills = min(len(skills), 15)
            n_skills = np.random.randint(5, max_possible_skills + 1)
